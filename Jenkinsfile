pipeline {
    ...
    stages {
        ...
        stage('Test & Verify') {
            steps {
                script{
                    docker.image('postgres:12_ja').withRun('-e "POSTGRES_DB=example" -e "POSTGRES_USER=postgres" -e "POSTGRES_PASSWORD=password" -e "POSTGRES_INITDB_ARGS=--encoding=UTF-8 --lc-collate=C --lc-ctype=ja_JP.UTF-8" -v "./docker/postgres/initdb.d:/docker-entrypoint-initdb.d" -v "./docker/postgres/conf.d:/etc/postgresql"') { c ->
                        ...
                        stage('Test') {
                            docker.image('azul/zulu-openjdk-alpine:8u202').inside("-v $HOME/.m2:/root/.m2:z -u root --link ${c.id}:db") {
                                sh "./mvnw clean test -Dspring.datasource.url=jdbc:postgresql://db:5432/example?example"
                            }
                        }
                        stage('Verify') {
                            docker.image('azul/zulu-openjdk-alpine:8u202').inside("-v $HOME/.m2:/root/.m2:z -u root --link ${c.id}:db") {
                                sh "./mvnw verify -Dspring.datasource.url=jdbc:postgresql://db:5432/example?example"
                            }
                        }
                    }
                }
