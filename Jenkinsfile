pipeline {
    ...
    stages {
        ...
        stage('Test & Verify') {
            steps {
                script{
                    docker.image('postgres:12_ja').withRun('
                        -e "POSTGRES_DB=example"
                        -e "POSTGRES_USER=postgres"
                        -e "POSTGRES_PASSWORD=password"
                        -e "POSTGRES_INITDB_ARGS=--encoding=UTF-8 --lc-collate=C --lc-ctype=ja_JP.UTF-8"
                        -v "./docker/postgres/initdb.d:/docker-entrypoint-initdb.d"
                        -v "./docker/postgres/conf.d:/etc/postgresql"
                    ') { c ->
